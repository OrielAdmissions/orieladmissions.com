@php
  $colleges = get_posts([
    ...
    $jumpLinks = collect($colleges)->map(fn ($c) => [
...
<div class="bg-chalk alignwide">
  <section class="pb-30">
    <div class="grid gap-x-8 md:grid-cols-12">
      <!-- Sticky Sidebar -->
      <aside id="college-nav" class="md:border-keyline/80 h-fit py-6 md:col-span-3 md:border-t md:py-10">
        <ul class="pin-content space-y-2 max-md:hidden">
          @foreach ($colleges as $college)
            ...
            <li>
              <a href="#{{ $id }}" class="hover:text-oriel text-xl transition">
                {{ $college->post_title }}
              </a>
            </li>
          @endforeach
        ...
        <!-- Mobile Dropdown -->
        <div class="dropdown-wrapper flex justify-center md:hidden" data-links='{{ $jumpLinks }}'>
          <div class="relative z-50 w-full" x-data="dropdown($el)">
            <!-- Button -->
            ...
            <div x-ref="panel" x-show="open" x-transition.origin.top.left x-on:click.outside="close($refs.button)"
              ...
              <template x-for="link in links" :key="link.href">
                <a :href="link.href" x-text="link.text" x-on:click="close()"
                   class="flex w-full items-center rounded-md px-2 py-2 text-left text-gray-800 transition-colors hover:bg-gray-50 focus-visible:bg-gray-50 lg:py-1.5"></a>
              </template>
            </div>
          </div>
        </div>
      ...
      <div
        ...
        @foreach ($colleges as $college)
          ...
          <div id="{{ $id }}">
            <x-college-card :college_name="$college->post_title" :description="$description"
                            :acceptance_rate="$acceptance_rate" :applicants="$applicants"
                            ...
                            :applicants_admitted_total="$applicants_admitted_total" :button_text="$button_text"
